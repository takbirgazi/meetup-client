...
import { MdCallEnd, MdOutlineAddReaction, MdOutlineChat, MdOutlinePoll } from "react-icons/md";
...
const Room = () => {
    ...
    const displayedUsers = joinUser?.length > MAX_USERS_DISPLAYED
        ? [...joinUser.slice(0, MAX_USERS_DISPLAYED - 1), { name: `${joinUser.length - MAX_USERS_DISPLAYED + 1}+`, imageUrl: image }]
        : joinUser;
    ...
    const buttonStyleMobile = "bg-gray-800 hover:bg-gray-600 px-4 py-3 rounded-full";
        ...
        <div className="bg-[#202124] min-h-screen flex flex-col">
            ...
            <div className="fixed bottom-0 h-20 w-full bg-transparent p-4 flex items-center justify-between text-gray-100">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <button className={buttonStyleMobile}><AiOutlineAudio /></button>
                    <button className={buttonStyleMobile}><IoVideocamOutline /></button>
                    <button className={buttonStyleMobile}><HiOutlineHandRaised /></button>
                    <button className={`btn w-20 text-lg rounded-full bg-red-500 hover:bg-gray-600 hover:text-red-500`}><MdCallEnd /></button>

                    {/* Hidden on Mobile, Visible on Large Screens */}
                    <div className="hidden lg:flex items-center space-x-2">
                        <button className={buttonStyleMobile}><TbShareplay /></button>
                        <button className={buttonStyleMobile}><MdOutlineAddReaction /></button>
                    </div>

                    {/* Options Button (Always Visible) */}
                    <button className={buttonStyleMobile} onClick={() => toggleDrawer("options")}>
                        <SlOptionsVertical />
                    </button>
                </div>
                ...
                <div className="flex items-center space-x-2 text-2xl">
                    <button className='md:hidden px-4 py-3 btn text-white bg-transparent hover:bg-gray-600' onClick={() => toggleDrawer("chatting")}>
                        <MdOutlineChat />
                    </button>
                    ...
                    <div className="hidden lg:flex space-x-2">
                        <div className="relative px-4 py-3 btn text-white bg-transparent hover:bg-gray-700 rounded-full">
                            ...
                            <button><TbUsers /></button>
                        </div>
                        <button className='px-4 py-3 btn text-white bg-transparent hover:bg-gray-600' onClick={() => toggleDrawer("chatting")}>
                            <MdOutlineChat />
                        </button>
                        <button className='px-4 py-3 btn text-white bg-transparent hover:bg-gray-600' onClick={() => toggleDrawer("polling")}>
                            <MdOutlinePoll />
                        </button>
                    </div>
                </div>
