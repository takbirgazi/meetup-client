...
export const MyUILayout = ({ showWhiteboard, setShowWhiteboard }) => {
  ...
  const handleNoteSubmit = async () => {
    if (note.trim()) {
      try {
        const response = await axiosSecure.post("/create-note", {
          text: note,
          email: participant?.email, // Assuming you have the user's email
        });
        console.log("Note Submitted:", response.data);
        setShowNoteModal(false); // Close the modal after submission
      ...
      {showNoteModal && (
        <div className="absolute right-20 bottom-20 flex bg-gray-700 p-4 rounded shadow-lg z-50 w-64">
          ...
          <button
            ...
            className="ml-2 flex items-center justify-center text-white p-2 rounded hover:bg-blue-300"
          >
            <LuCopyPlus />
          </button>
        </div>
